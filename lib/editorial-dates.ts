...
function requireCanonicalIsoDate(value: string): string {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) throw new Error(`Unsupported editorial date: ${value}`);
  const parsed = new Date(`${value}T00:00:00.000Z`);
  if (Number.isNaN(parsed.getTime()) || parsed.toISOString().slice(0, 10) !== value) throw new Error(`Invalid editorial calendar date: ${value}`);
  return value;
}
...
export function toRssEditorialDate(value: string): string {
  return new Date(`${toIsoEditorialDate(value)}T00:00:00.000Z`).toUTCString();
}
