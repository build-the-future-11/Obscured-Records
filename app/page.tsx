  ...
  return <main>
    ...
    <section className="paper-section latest-section"><SectionHeader number="01" title="Evidence files" note="Three events reconstructed from official reports, public records and research." />
      <div className="evidence-grid">{evidence.map((story, index) => <article key={story.slug} className={index === 0 ? "evidence-lead" : ""}>
        <Link href={`/article/${story.slug}`} aria-label={`Read ${story.title}`} className={`evidence-image ${story.cover ? "has-cover" : `visual-${index + 1}`}`} style={story.cover ? {backgroundImage:`linear-gradient(180deg,transparent,rgba(10,10,10,.42)),url('${story.cover}')`} : undefined}><RecordId id={story.recordId} inverse /></Link>
        <StoryMeta article={story} /><h2><Link href={`/article/${story.slug}`}>{story.title}</Link></h2><p>{story.excerpt}</p>
      </article>)}</div>
    </section>
    <section className="dark-section underreported-section"><SectionHeader number="02" title="Deep record" note={`${dossiers.length} expanded features. Each one includes a timeline, multiple source types and a clear account of uncertainty.`} inverse />
    ...
    <section className="paper-section most-read"><SectionHeader number="04" title="Editor's desk" note="Five concise records selected for the strength of the question they leave behind." />
      <ol>{desk.map((story, index) => <li key={story.slug}><span className="rank">0{index + 1}</span><div><span className="kicker">{story.section} / Brief record</span><h3><Link href={`/article/${story.slug}`}>{story.title}</Link></h3></div><span className="read-time">{getReadingLabel(story.slug).split(" · ").at(-1)}</span><span className={`thumb visual-${index + 1}`} /></li>)}</ol>
    </section><NewsletterCTA /><Footer />
  </main>;
}
